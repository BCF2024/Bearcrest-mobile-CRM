const CONFIG = {
  SPREADSHEET_ID: '1GJJHJNX-Wkscf5jGi5HV2SkwL-1UUPPBs9O-0I3PBdo',
  ROOT_DRIVE_FOLDER_ID: '1tOeYa47DBUbWtnXuPS8N5Ulw3wYkOunQ',
  LOANS_SHEET: 'Loans',
...
function doGet() {
  return json_({ok:true,data:{message:'BearCrest CRM V8.3 backend is live'}});
}
...
function doPost(e) {
  try {
    ...
    switch (action) {
      case 'ping':
        data = {version:'8.3', message:'BearCrest CRM V8.3 backend is live'};
        break;
      case 'getAll':
        data = getAll_();
        break;
      case 'getNextLoanNumber':
        data = getNextLoanNumber_();
        break;
      case 'saveLoan':
        data = saveLoan_(payload.loan || {}, payload.applicationId || '');
        break;
      case 'deleteLoan':
        data = deleteLoan_(payload.recordId);
        break;
      case 'syncApplications':
        data = syncApplications_();
        break;
      case 'getDocuments':
        data = getDocuments_(payload.recordId);
        break;
      case 'uploadDocument':
        data = uploadDocument_(payload);
        break;
      case 'deleteDocument':
        data = deleteDocument_(payload.fileId);
        break;
      case 'addLender':
        data = addLender_(payload.name);
        break;
      case 'renameLender':
        data = renameLender_(payload.oldName, payload.newName);
        break;
      case 'deleteLender':
        data = deleteLender_(payload.name);
        break;
      default:
...
function setupBearCrestCRM() {
  ...
  if (lendersSheet.getLastRow() < 2) {
    const defaultLenders = getDefaultLenders_();
    lendersSheet.getRange(2,1,defaultLenders.length,2)
      .setValues(defaultLenders.map(function(name) {
        return [name, 'Yes'];
      }));
  }
  ...
  return 'BearCrest CRM V8.3 setup complete.';
}
...
function getLenders_() {
  const fallback = getDefaultLenders_();
  const ss = SpreadsheetApp.openById(CONFIG.SPREADSHEET_ID);
  ...
  if (sh.getLastRow() < 2) {
    sh.getRange(2,1,fallback.length,2)
      .setValues(fallback.map(function(name) {
        return [name,'Yes'];
      }));
  }
  ...
  let nameIdx = headers.findIndex(function(h) {
    return ['lender name','lender','name'].indexOf(h) !== -1;
  });

  let activeIdx = headers.findIndex(function(h) {
    return ['active','status','enabled'].indexOf(h) !== -1;
  });
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    const active = activeIdx >= 0
      ? String(data[i][activeIdx] || '').trim().toLowerCase()
      : 'yes';

    if (
      name &&
      ['no','false','inactive','disabled','0'].indexOf(active) === -1
    ) {
      out.push({name:name});
    }
  }
...
function addLender_(name) {
  ...
  if (!name) {
    throw new Error('Enter a lender name.');
  }
...
function renameLender_(oldName, newName) {
  ...
  if (!oldName || !newName) {
    throw new Error('Both lender names are required.');
  }
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    if (
      ...
      existing.toLowerCase() !== oldName.toLowerCase()
    ) {
      throw new Error('That lender already exists.');
    }
  }
...
function saveLoan_(loan, applicationId) {
  ...
  if (!loan.recordId) {
    loan.recordId = Utilities.getUuid();
  }

  let rowIndex = -1;

  for (let i = 1; i < values.length; i++) {
    if (String(values[i][0]) === String(loan.recordId)) {
      rowIndex = i + 1;
    }
  }
  ...
  if (rowIndex > 0) {
    existing = rowToObject_(headers, values[rowIndex - 1]);
  }

  const folderId = existing.driveFolderId || createLoanFolder_(loan);
  const record = {};

  LOAN_HEADERS.forEach(function(h) {
    record[h] = loan[h] !== undefined ? loan[h] : (existing[h] || '');
  });
  ...
  const row = LOAN_HEADERS.map(function(h) {
    return record[h];
  });
  ...
  if (applicationId) {
    markApplicationImported_(applicationId);
  }
  ...
  log_(
    ...
    record.recordId,
    record.loanNumber,
    record.borrowerName + ' | ' + record.status
  );
...
function syncApplications_() {
  ...
  return {
    unimported:getUnimportedApplications_().length
  };
}
...
function getUnimportedApplications_() {
  ...
  for (let r = 1; r < data.length; r++) {
    ...
    if (
      importedIdx >= 0 &&
      String(row[importedIdx]).toLowerCase() === 'yes'
    ) {
      continue;
    }
    ...
    if (!applicationId) {
      ...
      if (appIdIdx >= 0) {
        sh.getRange(r + 1, appIdIdx + 1).setValue(applicationId);
      }
    }
...
function uploadDocument_(p) {
  if (!p.recordId || !p.base64 || !p.fileName) {
    throw new Error('Missing upload information.');
  }

  const loan = readObjects_(CONFIG.LOANS_SHEET).find(function(item) {
    return String(item.recordId) === String(p.recordId);
  });
  ...
  const folder = DriveApp.getFolderById(
    loan.driveFolderId || createLoanFolder_(loan)
  );
...
function ensureReady_() {
  if (
    CONFIG.SPREADSHEET_ID.indexOf('PASTE_') === 0 ||
    CONFIG.ROOT_DRIVE_FOLDER_ID.indexOf('PASTE_') === 0
  ) {
    throw new Error(
      'Add your Google Sheet ID and Drive folder ID in CONFIG, then run setupBearCrestCRM().'
    );
  }
}
...
function ensureApplicationTrackingColumns_() {
  ...
  if (!headers.includes('Imported')) {
    sh.getRange(1,sh.getLastColumn() + 1).setValue('Imported');
  }
  ...
  if (!updated.includes('Application ID')) {
    sh.getRange(1,sh.getLastColumn() + 1).setValue('Application ID');
  }
}
...
function log_(action,recordId,loanNumber,details) {
  ...
  ss
    .getSheetByName(CONFIG.ACTIVITY_SHEET)
    .appendRow([
      new Date(),
      action,
      recordId,
